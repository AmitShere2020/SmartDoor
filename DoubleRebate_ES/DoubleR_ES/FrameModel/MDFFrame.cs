...
namespace DoubleR_ES.FrameModel
{
    public class MDFFrame : Frame
    {
        ...
        private List<Point3D> GetSlotPoints(double length, double tabBase, double slotHeight, double slotWidth)
        {
            ...
            double btmY ;//= (Utilities.InputData.Return1 - JsonData.BendDataList[0].BendAllowance) +
                         //(Utilities.InputData.Architrave1 - JsonData.BendDataList[1].BendAllowance)
                         //+ factor1 + (tabBase / 2) - (slotHeight / 2);// corner point so tab base minus

             if (Utilities.InputData.Return1 != 0)
             {
                 btmY = rightProfileBottom.Y + RequiredList[1].Line.EndPoint.Y;
                 btmY = btmY + factor1 + (tabBase / 2) + (slotHeight / 2);
             }
             else
             {
                 btmY = RequiredList[0].Line.EndPoint.Y + factor2 + (tabBase / 2) + (slotHeight / 2);
             }

            double topX = btmX;

            double temp=0;
            if (Utilities.InputData.Return2 != 0 && Utilities.InputData.Architrave2 != 0)
            {
                temp =RequiredList[7].Line.StartPoint.Y - factor2 + (tabBase / 2) + (slotHeight / 2);
                
            }
            else
            {
                temp = rightProfileTop.Y - factor2 + (tabBase / 2) + (slotHeight / 2);
            }
            ...
            slotBasePointList.Add(new Point3D { X = topX, Y = temp });
